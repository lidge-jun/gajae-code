# 060_goal — 02 logic changes (gjc_origin)

> gjc_origin: upstream **기준 동작** (변경 전). 클론 @ `75d103f45145`.

## upstream 기준 동작

- `ultragoal` SKILL/엔진; standalone `goal` CLI surface는 포크 전 제한.

## fork에서 바뀐 점

→ [jwc_patched/060_goal/02_logic_changes.md](../../jwc_patched/060_goal/02_logic_changes.md)

## diff

`diff -u devlog/_upstream_gjc/packages/... packages/...` — 경로: [02_code_facts.md](./02_code_facts.md)

## 정본

- 횡단: [structure/40_fork-delta.md](../../../structure/40_fork-delta.md)
- 파일 단위: [structure/40_fork-delta.md](../../../structure/40_fork-delta.md)
- 앵커 경로: [02_code_facts.md](./02_code_facts.md)

