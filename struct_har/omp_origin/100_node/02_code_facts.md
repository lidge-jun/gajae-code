# 100_node — 02 code facts (omp_origin)
...
| 2 | `packages/agent` | present |
| 3 | `packages/ai` | present |

## diff (예)

```bash
diff -u devlog/_upstream_omp/packages/coding-agent/src/cli.ts devlog/_upstream_gjc/packages/coding-agent/src/cli.ts | head
```
