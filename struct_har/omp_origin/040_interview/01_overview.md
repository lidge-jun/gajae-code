# 040_interview — 01 overview (omp_origin)

> omp @ `db421bb2ef68` · 클론 [`devlog/_upstream_omp/`](../../../devlog/_upstream_omp/)
> MOC (jaw 밴드 정렬): [`040_moc_interview_merge.md`](../../../devlog/_plan/260612_jawcode_fork/phase1/040_moc_interview_merge.md)

## 요구사항 인터뷰

- gjc식 **`deep-interview` 번들 스킬 없음**.
- 요구사항 명확화는 일반 세션 + 사용자 스킬/프롬프트에 위임.

## jawcode 대조

jwc: `jaw-interview` native + gate + structured ask (040 fork).

| 문서 | |
|---|---|
| [02_code_facts.md](./02_code_facts.md) | omp 경로 앵커 |
| [02_logic_changes.md](./02_logic_changes.md) | omp 동작 요약 (fork 아님) |
| [03_devlog_refs.md](./03_devlog_refs.md) | 플랜 링크 |

횡단: [../README.md](../README.md) · [../architecture/](../architecture/) · [structure/fork_logic_changelog.md](../../../structure/fork_logic_changelog.md)
