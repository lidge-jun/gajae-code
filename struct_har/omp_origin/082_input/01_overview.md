# 082_input — 01 overview (omp_origin)

> omp @ `db421bb2ef68` · 클론 [`devlog/_upstream_omp/`](../../../devlog/_upstream_omp/)
> MOC (jaw 밴드 정렬): [`082_moc_tui_input.md`](../../../devlog/_plan/260612_jawcode_fork/phase1/082_moc_tui_input.md)

## TUI 입력 · IME

- `packages/tui/` 키·IME·에디터 — upstream omp 동작.

## jawcode 대조

jwc: Ctrl-chord 한글, caret fix (082 fork).

| 문서 | |
|---|---|
| [02_code_facts.md](./02_code_facts.md) | omp 경로 앵커 |
| [02_logic_changes.md](./02_logic_changes.md) | omp 동작 요약 (fork 아님) |
| [03_devlog_refs.md](./03_devlog_refs.md) | 플랜 링크 |

횡단: [../README.md](../README.md) · [../architecture/](../architecture/) · [structure/fork-delta.md](../../../structure/fork-delta.md)
