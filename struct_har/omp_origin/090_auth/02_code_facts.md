# 090_auth — 02 code facts (omp_origin)
...
| 2 | `packages/catalog` | present |

## diff (예)

```bash
diff -u devlog/_upstream_omp/packages/ai/src/auth-storage.ts devlog/_upstream_gjc/packages/ai/src/auth-storage.ts | head
```
