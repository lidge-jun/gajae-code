...
| **M2 Node 포팅 — 100밴드 완료 (260613)** | `packages/jwc/dist-node/`(esbuild 번들) + `packages/jwc/src/shims/`: global Bun shim, file/write/sleep/stdio, `Bun.spawn`/`spawnSync` Node 어댑터, data-core(`bun:sqlite`/hash/JSONL/JSON5/stripANSI), peripheral shims. Node 22 SDK import·`createAgentSession`·스트리밍 green, 적대 감사 라운드 1-4 통과(보안: archive long-name tar path traversal·archive mtime·serve TLS/disconnect 포함, `0debe38b`). | "문서만"에서 **구현 완료**로 전환 — Bun API가 dist-node 셰임으로 대체됨. | `packages/jwc/scripts/build-node.ts`, `packages/jwc/src/shims/`, devlog 100밴드 (`2e9efc59`…`fba5cd56`, closeout `fdb8d41d`) |
