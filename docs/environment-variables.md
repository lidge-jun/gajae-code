...
> **Legacy aliases**: every `JWC_*` variable below also accepts its `GJC_*` legacy spelling (read-fallback, 062.1). New configurations should use `JWC_*`.
...
`gjc ralplan`, `gjc deep-interview`, and `gjc state` are private runtime bridge commands. They require `JWC_RUNTIME_BINARY` (or legacy `JWC_LEGACY_RUNTIME_BINARY`) to point at the private runtime executable; public bundled workflow use remains through `/skill:ralplan` and `/skill:deep-interview` inside a GJC session.

| Variable | Behavior |
| --- | --- |
| `JWC_RUNTIME_BINARY` | Private runtime bridge binary for `gjc ralplan`, `gjc deep-interview`, and `gjc state` |
| `JWC_LEGACY_RUNTIME_BINARY` | Legacy fallback bridge binary name |
...
| `JWC_TEAM_WORKER_CLI_MAP` | Comma-separated worker CLI selector map; entries must be `auto` or `gjc` |
...
| `JWC_COORDINATOR_MCP_SETUP_MANAGED_BY` | Marker written by `gjc setup hermes` for safe managed config updates. |
| `JWC_COORDINATOR_MCP_SETUP_SCHEMA_VERSION` | Managed setup schema version written by `gjc setup hermes`. |
| `JWC_COORDINATOR_MCP_SETUP_SIGNATURE` | Deterministic managed setup signature used to detect safe updates versus unmanaged conflicts. |
...
| Variable                             | Behavior                                             |
| ------------------------------------ | ---------------------------------------------------- |
| `JWC_OPENAI_CODE_DEBUG`                     | `1`/`true` enables OpenAI code provider debug logging      |
| `JWC_OPENAI_CODE_WEBSOCKET`                 | `1`/`true` enables websocket transport preference    |
| `JWC_OPENAI_CODE_WEBSOCKET_V2`              | `1`/`true` enables websocket v2 path                 |
| `JWC_OPENAI_CODE_WEBSOCKET_IDLE_TIMEOUT_MS` | Positive integer override (default 300000)           |
| `JWC_OPENAI_CODE_WEBSOCKET_RETRY_BUDGET`    | Non-negative integer override (default 5)            |
| `JWC_OPENAI_CODE_WEBSOCKET_RETRY_DELAY_MS`  | Positive integer base backoff override (default 500) |
| `JWC_OPENAI_STREAM_IDLE_TIMEOUT_MS`   | Positive integer OpenAI stream idle timeout override |
...
| Variable             | Behavior                                                                                                          |
| -------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `JWC_CACHE_RETENTION` | If `long`, enables long retention where supported (`anthropic`, `openai-responses`, Bedrock retention resolution) |
...
| `JWC_OPENAI_CODE_WEB_SEARCH_MODEL`                         | OpenAI code search provider model override                          |
...
| Variable            | Behavior                                                                        |
| ------------------- | ------------------------------------------------------------------------------- |
| `JWC_AUTH_NO_BORROW` | If set, disables macOS native-app token borrowing path in Perplexity login flow |
...
| Variable                  | Default / behavior                                                                                                  |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `JWC_PY`                   | Eval backend override: `0`/`bash`=JavaScript only, `1`/`py`=Python only, `mix`/`both`=both; invalid values ignored |
| `JWC_PYTHON_SKIP_CHECK`    | If `1`, skips Python interpreter availability checks (subprocess runner still starts on demand)                     |
| `JWC_PYTHON_INTEGRATION`   | If `1`, opts gated integration tests in (e.g. `python-runner.integration.test.ts`) into running against real Python |
| `JWC_PYTHON_IPC_TRACE`     | If `1`, logs NDJSON frames exchanged with the Python runner subprocess                                              |
...
- Python env filtering denies common API keys and allows safe base vars + `LC_`, `XDG_`, `JWC_` prefixes.
...
| Variable                     | Default / behavior                                                                                 |
| ---------------------------- | -------------------------------------------------------------------------------------------------- |
| `JWC_SMOL_MODEL`              | Ephemeral model-role override for `smol` (CLI `--smol` takes precedence)                           |
| `JWC_SLOW_MODEL`              | Ephemeral model-role override for `slow` (CLI `--slow` takes precedence)                           |
| `JWC_PLAN_MODEL`              | Ephemeral model-role override for `plan` (CLI `--plan` takes precedence)                           |
| `JWC_NO_TITLE`                | If set (any non-empty value), disables auto session title generation on first user message         |
| `NULL_PROMPT`                | If `true`, system prompt builder returns empty string                                              |
| `JWC_BLOCKED_AGENT`           | Blocks a specific subagent type in task tool                                                       |
| `JWC_SUBPROCESS_CMD`          | Overrides subagent spawn command (`gjc` / `gjc.cmd` resolution bypass)                             |
| `JWC_TASK_MAX_OUTPUT_BYTES`   | Max captured output bytes per subagent (default `500000`)                                          |
| `JWC_TASK_MAX_OUTPUT_LINES`   | Max captured output lines per subagent (default `5000`)                                            |
| `JWC_TIMING`                  | If set (any non-empty value), prints a hierarchical timing-span tree to **stderr** via `logger.printTimings()`. In interactive mode the tree prints once the agent is ready (before the TUI starts); in print mode it prints after the whole prompt batch completes. Print-mode prompts are wrapped in `print:prompt:initial` / `print:prompt:next` spans so each user message shows up as its own row. `JWC_TIMING=x` exits the process with code 0 right after printing in interactive mode (use to measure cold startup only). `JWC_TIMING=full` lists every module-load entry instead of just the top N. |
| `JWC_PACKAGE_DIR`             | Overrides package asset base dir resolution (docs/examples/changelog path lookup)                  |
| `JWC_DISABLE_LSPMUX`          | If `1`, disables lspmux detection/integration and forces direct LSP server spawning                |
| `JWC_RPC_EMIT_TITLE`          | Boolean-like flag enabling title events in RPC mode                                                |
...
| `JWC_EDIT_VARIANT`            | Forces edit tool variant when valid (`patch`, `replace`, `hashline`, `atom`, `vim`, `apply_patch`) |
| `JWC_FORCE_IMAGE_PROTOCOL`    | Forces supported image protocol (`kitty`, `iterm2`/`iterm`, `sixel`, `none`) where used            |
| `JWC_ALLOW_SIXEL_PASSTHROUGH` | Allows SIXEL passthrough when `JWC_FORCE_IMAGE_PROTOCOL=sixel`                                      |
| `JWC_NO_PTY`                  | If `1`, disables interactive PTY path for bash tool                                                |

`JWC_NO_PTY` is also set internally when CLI `--no-pty` is used.
...
| Variable                   | Behavior                                                                       |
| -------------------------- | ------------------------------------------------------------------------------ |
| `JWC_BASH_NO_CI`            | Suppresses automatic `CI=true` injection into spawned shell env                |
| `ANTHROPIC_MODEL_BASH_NO_CI`        | Legacy alias fallback for `JWC_BASH_NO_CI`                                      |
| `JWC_BASH_NO_LOGIN`         | Disables login-shell mode; shell args become `['-c']` instead of `['-l','-c']` |
| `ANTHROPIC_MODEL_BASH_NO_LOGIN`     | Legacy alias fallback for `JWC_BASH_NO_LOGIN`                                   |
| `JWC_SHELL_PREFIX`          | Optional command prefix wrapper                                                |
| `ANTHROPIC_MODEL_CODE_SHELL_PREFIX` | Legacy alias fallback for `JWC_SHELL_PREFIX`                                    |
...
| Variable                  | Behavior                                                                              |
| ------------------------- | ------------------------------------------------------------------------------------- |
| `JWC_NOTIFICATIONS`        | `off` / `0` / `false` suppress desktop notifications                                  |
| `JWC_TUI_WRITE_LOG`        | If set, logs TUI writes to file                                                       |
| `JWC_HARDWARE_CURSOR`      | If `1`, enables hardware cursor mode                                                  |
| `JWC_CLEAR_ON_SHRINK`      | If `1`, clears empty rows when content shrinks                                        |
| `JWC_DEBUG_REDRAW`         | If `1`, enables redraw debug logging                                                  |
| `JWC_TUI_DEBUG`            | If `1`, enables deep TUI debug dump path                                              |
| `JWC_FORCE_IMAGE_PROTOCOL` | Forces terminal image protocol detection (`kitty`, `iterm2`/`iterm`, `sixel`, `none`) |
...
| Variable                  | Behavior                                                            |
| ------------------------- | ------------------------------------------------------------------- |
| `JWC_COMMIT_TEST_FALLBACK` | If `true` (case-insensitive), force commit fallback generation path |
| `JWC_COMMIT_NO_FALLBACK`   | If `true`, disables fallback when agent returns no proposal         |
| `JWC_COMMIT_MAP_REDUCE`    | If `false`, disables map-reduce commit analysis path                |
...
| --- | --- | --- | --- |
| `JWC_BRIDGE_TOKEN` | Yes | — | Bearer token required on authenticated endpoints. **Secret — never commit.** |
| `JWC_BRIDGE_TLS_CERT` | Yes | — | Path to the TLS certificate (PEM). Startup fails closed if cert/key are missing (TLS is mandatory, including loopback). |
| `JWC_BRIDGE_TLS_KEY` | Yes | — | Path to the TLS private key (PEM). **Secret — never commit; `chmod 600`.** |
| `JWC_BRIDGE_HOST` | No | `127.0.0.1` | Bind hostname. |
| `JWC_BRIDGE_PORT` | No | `4077` | Bind port (1–65535). |
| `JWC_BRIDGE_SCOPES` | No | `prompt` | Parsed for dormant command-surface compatibility. Valid scopes: `prompt`, `control`, `bash`, `export`, `session`, `model`, `message:read`, `host_tools`, `host_uri`, `admin`. The default endpoint matrix still advertises no accepted scopes and rejects commands before scope checks. |
...
- Bridge auth/TLS material (`JWC_BRIDGE_TOKEN` and the `JWC_BRIDGE_TLS_KEY` private key; never commit cert/key/token material)
