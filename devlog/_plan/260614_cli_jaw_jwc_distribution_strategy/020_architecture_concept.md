...
- `README.jwc.md`: jwc is "cli-jaw's native agent runtime" and `jwc/sdk` is the single import path for cli-jaw.
...
| `jwc/sdk` | JWC public embedding API | stable runtime/session factory consumed by cli-jaw |
| `cli-jaw` repo | orchestration/product shell | bundles or vendors JWC runtime; no global `jwc` requirement |
...
cli-jaw integrated mode must not shell out to a user-installed `jwc` binary as the primary path. It may keep shell fallback for rollback, but the product goal is:
...
- jawcode can release standalone jwc on its own cadence.
- cli-jaw can vendor, bundle, or package-depend on a versioned JWC artifact.
