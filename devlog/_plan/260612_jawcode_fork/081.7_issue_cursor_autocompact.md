...
> 상태: 🔍 원인 확정(신뢰도 90%) / 수정 ⬜. 조사: 2026-06-12 11시. 소속: 081 cursor 도구군 ([081_moc_cursor_tools.md](./081_moc_cursor_tools.md)).
...
- cursor 모델로 긴 세션 → 임계치(기본 ~90%) 도달 시 자동 compact 발동 e2e. 상태줄 %와 compact 트리거 일치.
- 단위: `shouldCompact` 입력을 estimate로 바꾼 경로에 cursor usage(input:0) 픽스처로 트리거 확인.
