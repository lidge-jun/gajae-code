# 083 — issue: cursor 도구 실행이 전부 실패 (exec 핸들러 unbound this)
...
> 소속: 080 밴드 (cursor 파이프라인). 선행: [081](./081_issue_tui_toolcall_render.md)(렌더)·[082](./082_issue_cursor_tools_fail.md)(타이틀 환각).
...
- Glob 호출 시 "Pattern must not be empty" → **수정 완료**: grep 핸들러에서 빈 pattern이면 `find`로
  라우팅 (`packages/coding-agent/src/cursor.ts`). 종합: [084](./084_fix_cursor_tools_resolution.md).
