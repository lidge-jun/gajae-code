...
const CU_NATIVE_PATH =
  process.env["CU_NATIVE_PATH"] ??
  resolve(__dirname, "..", "bin", "cu-native");
...
export const native = {
  ...
  key: (combo: string, repeat?: number) =>
    callNative([
      "key",
      "--combo", combo,
      ...(repeat != null ? ["--repeat", String(repeat)] : []),
    ]),
  ...
  appsUnderPoint: (x: number, y: number) =>
    callNative(["apps", "under-point", `--x=${x}`, `--y=${y}`]),

  appsHide: (bundleIds: string[]) =>
    callNative(["apps", "hide", "--bundle-ids", bundleIds.join(",")]),

  appsUnhide: (bundleIds: string[]) =>
    callNative(["apps", "unhide", "--bundle-ids", bundleIds.join(",")]),

  appsOpen: (app: string) =>
    callNative(["apps", "open", "--app", app]),

  appsResolve: (names: string[]) =>
    callNative(["apps", "resolve", "--names", names.join(",")]),

  // --- Display (nested subcommands) ---
  displayList: () => callNative(["display", "list"]),

  displaySize: (id?: number) =>
    callNative([
      "display",
      "size",
  ...
  clipboardWrite: (text: string) =>
    callNative(["clipboard", "write", "--text", text]),
