...
const APPLESCRIPT_SUFFIX = " Do not attempt to work around this restriction — never use AppleScript, System Events, shell commands, or any other method to send clicks or keystrokes to this app.";
...
export const ERRORS = {
  ...
  NO_ALLOWLIST: () =>
    `No apps in allowlist. Call request_access first.`,

  CLIPBOARD_READ_REQUIRED: () =>
    `Requires the \`clipboardRead\` grant.`,

  CLIPBOARD_WRITE_REQUIRED: () =>
    `Requires the \`clipboardWrite\` grant.`,

  MOUSE_ALREADY_HELD: () =>
    `Mouse button is already held. Call left_mouse_up first.`,
