...
import { GrantedApp, SessionState } from "../session.js";
import { getAppCategory, categoryToTier, isFullTierOverride, type AppCategory } from "../safety/tiers.js";
import { MEDIA_BUNDLE_IDS } from "../safety/bundleIds.js";
...
export function registerAppTools(server: McpServer, session: SessionState) {
  server.registerTool(
    ...
    {
      description:
        "Bring an application to the front, launching it if necessary. The app must already be granted.",
    ...
    async ({ app }) => {
      ...
      if (!target || !target.installed || !target.bundleId) {
        return fail(`Application "${app}" is not installed.`);
      }
      if (!session.allowedApps.has(target.bundleId)) {
        return fail(
          `Application "${target.name}" is not in the allowed applications. Call request_access first.`,
        );
      }
  ...
  server.registerTool(
    ...
    async ({ apps, reason, clipboardRead, clipboardWrite, systemKeyCombos }) => {
      ...
      session.grantFlags.clipboardRead =
        session.grantFlags.clipboardRead || Boolean(clipboardRead);
      session.grantFlags.clipboardWrite =
        session.grantFlags.clipboardWrite || Boolean(clipboardWrite);
      session.grantFlags.systemKeyCombos =
        session.grantFlags.systemKeyCombos || Boolean(systemKeyCombos);
  ...
  server.registerTool(
    ...
    {
      description:
        "List the currently granted applications plus active grant flags and coordinate mode.",
      inputSchema: z.object({}),
    },
    async () => {
      const response = {
