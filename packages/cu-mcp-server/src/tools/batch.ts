...
import { ScreenshotMeta, SessionState } from "../session.js";
...
function convertCursorPosition(
  ...
  switch (action.action) {
    case "key": {
      ...
      if (isSystemKeyCombo(text) && !session.grantFlags.systemKeyCombos) {
        throw new Error(`"${text}" is a system-level shortcut. Request the \`systemKeyCombos\` grant via request_access to use it.`);
      }
    ...
    case "triple_click": {
      const category: ActionCategory =
        action.action === "right_click" || action.action === "middle_click" || action.text
          ? "mouse_full" : "mouse";
      const tierError = await enforcePreAction(session, category);
      if (tierError) throw new Error(tierError);
      const coordinate = requireCoordinate(action);
      const resolved = resolveCoordinate(coordinate[0], coordinate[1], frozenState);
      const pointError = await enforcePointUnderClick(session, resolved.x, resolved.y, category === "mouse_full" ? "mouse_full" : "mouse");
      if (pointError) throw new Error(pointError);
      const config =
...
export function registerBatchTool(server: McpServer, session: SessionState) {
  server.registerTool(
    ...
    {
      description:
        "Execute a predictable sequence of computer-use actions in one tool call.",
