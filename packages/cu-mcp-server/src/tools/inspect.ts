...
import { enforcePreAction, enforcePointUnderClick } from "../safety/enforcement.js";
    ...
    "inspect",
    {
      ...
      inputSchema: z.object({
        coordinate: z
          ...
          .describe("(x, y) in screenshot pixel coordinates."),
      }),
    },
    async ({ coordinate }) => {
  ...
  server.registerTool(
    ...
    async ({ coordinate }) => {
      ...
      try {
        ...
        return fail(`AX press failed at (${x}, ${y}). Element: ${result?.inspect?.role ?? "unknown"} "${result?.inspect?.title ?? ""}". Actions: ${(result?.inspect?.actions ?? []).join(", ")}`);
      } catch (err: any) {
