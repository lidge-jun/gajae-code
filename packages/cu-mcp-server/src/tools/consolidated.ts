...
import { SessionState, type ScreenshotMeta } from "../session.js";
import { enforcePreAction, enforcePointUnderClick, type ActionCategory } from "../safety/enforcement.js";
import { isSystemKeyCombo, getAppCategory, categoryToTier, isFullTierOverride } from "../safety/tiers.js";
...
type DisplayInfo = { id: number; originX: number; originY: number; width: number; height: number; isMain: boolean; name?: string };
type CaptureResult = { image: string; width: number; height: number; displayWidth: number; displayHeight: number; originX: number; originY: number };
...
function ok(t: string): ToolResponse { return { content: [{ type: "text", text: t }] }; }
function fail(t: string): ToolResponse { return { content: [{ type: "text", text: t }], isError: true }; }
function img(base64: string): ImageContent { return { type: "image", data: base64, mimeType: "image/jpeg" }; }
  ...
  try {
    ...
    if (frontmost) {
      for (const d of displays) {
        try {
          const app = (await native.appsUnderPoint(d.originX + d.width / 2, d.originY + d.height / 2)) as { bundleId: string } | null;
    ...
    for (const d of displays) {
      if (mouse.x >= d.originX && mouse.x < d.originX + d.width && mouse.y >= d.originY && mouse.y < d.originY + d.height) return d.id;
    }
  } catch {}
  return undefined;
}
...
const ACTIONS = [
  "capture", "click", "drag", "mouse_move", "mouse_down", "mouse_up",
  "scroll", "type", "key", "hold_key", "wait", "cursor_position",
  "switch_display", "inspect", "ax_press", "open_app", "request_access",
  "list_apps", "clipboard_read", "clipboard_write",
] as const;

const BATCH_ACTIONS = [
  "click", "drag", "mouse_move", "mouse_down", "mouse_up", "scroll",
  "type", "key", "hold_key", "wait", "cursor_position", "capture",
] as const;

const coordinate = () => z.tuple([z.number(), z.number()]).describe(
...
const inputSchema = z.object({
  action: z.enum([...ACTIONS, "batch"]).describe(
  ...
  // capture
  mode: z.enum(["screenshot", "zoom"]).optional().describe("capture: 'screenshot' (default) or 'zoom' for region crop."),
  region: z.tuple([z.number(), z.number(), z.number(), z.number()]).optional().describe("capture mode=zoom: [x0,y0,x1,y1] from last screenshot."),
  ...
  count: z.union([z.literal(1), z.literal(2), z.literal(3)]).optional().describe("click: 1=single, 2=double, 3=triple."),
...
async function handleCapture(input: Input, session: SessionState): Promise<ToolResponse> {
  if (input.mode === "zoom") {
    ...
    const x0 = Math.min(rawX0, rawX1), y0 = Math.min(rawY0, rawY1);
    const x1 = Math.max(rawX0, rawX1), y1 = Math.max(rawY0, rawY1);
    const scaleX = meta.displayWidth / meta.width, scaleY = meta.displayHeight / meta.height;
    const result = (await native.screenshotRegion({
      displayId: await autoDisplayId(session),
      allowedBundleIds: [...session.allowedApps.keys()],
      x: meta.originX + x0 * scaleX, y: meta.originY + y0 * scaleY,
      width: Math.max(1, (x1 - x0) * scaleX), height: Math.max(1, (y1 - y0) * scaleY),
      outWidth: 1200, outHeight: 900, quality: 0.85,
    })) as CaptureResult;
  ...
  const result = (await native.screenshot({
    displayId: await autoDisplayId(session),
    allowedBundleIds: [...session.allowedApps.keys()],
    quality: 0.75, maxWidth: 1568, maxHeight: 1568,
  })) as CaptureResult;
  session.lastScreenshot = { width: result.width, height: result.height, displayWidth: result.displayWidth, displayHeight: result.displayHeight, originX: result.originX, originY: result.originY };
  ...
  if (rawDisplays.length > 1) {
    ...
    const others = displays.filter(d => d !== captured).map(d => `"${d.name}"`).join(", ");
    const note = captured
...
async function handleSwitchDisplay(input: Input, session: SessionState): Promise<ToolResponse> {
  ...
  const match = displays.find(d => d.name?.toLowerCase() === q) ?? displays.find(d => d.name?.toLowerCase().includes(q));
  if (!match) {
...
async function handleOpenApp(input: Input, session: SessionState): Promise<ToolResponse> {
  ...
  const resolved = (await native.appsResolve([input.app])) as Array<{ name: string; bundleId: string | null; installed: boolean }>;
  const target = resolved[0];
  if (!target || !target.installed || !target.bundleId) return fail(`Application "${input.app}" not found.`);
  if (!session.allowedApps.has(target.bundleId)) return fail(`"${target.name}" is not granted. Call request_access first.`);
...
async function handleRequestAccess(input: Input, session: SessionState): Promise<ToolResponse> {
  ...
  const resolved = (await native.appsResolve(input.apps)) as Array<{ name: string; bundleId: string | null; installed: boolean }>;
  ...
  for (const entry of resolved) {
    if (!entry.installed || !entry.bundleId) { denied.push({ bundleId: entry.bundleId ?? entry.name, reason: "not_installed" }); continue; }
    const category = getAppCategory(entry.bundleId);
    const tier = categoryToTier(category);
    if (!tier) { denied.push({ bundleId: entry.bundleId, reason: "media_blocked" }); continue; }
...
async function handleListApps(session: SessionState): Promise<ToolResponse> {
  ...
  return ok(JSON.stringify({ granted, grantFlags: session.grantFlags, coordinateMode: session.coordinateMode }, null, 2));
}
...
async function handleBatch(input: Input, session: SessionState): Promise<ToolResponse> {
  ...
  for (const sub of input.actions) {
    try {
      ...
      completed.push({ action: sub.action, ok: false, output: error instanceof Error ? error.message : String(error) });
...
async function dispatch(input: Input, session: SessionState): Promise<ToolResponse> {
  switch (input.action) {
    case "capture": return handleCapture(input, session);
    case "click": return handleClick(input, session);
    case "drag": return handleDrag(input, session);
    case "mouse_move": return handleMouseMove(input, session);
    case "mouse_down": return handleMouseDown(session);
    case "mouse_up": return handleMouseUp(session);
    case "scroll": return handleScroll(input, session);
    case "type": return handleType(input, session);
    case "key": return handleKey(input, session);
    case "hold_key": return handleHoldKey(input, session);
    case "wait": return handleWait(input);
    case "cursor_position": return handleCursorPosition(session);
    case "switch_display": return handleSwitchDisplay(input, session);
    case "inspect": return handleInspect(input, session);
    case "ax_press": return handleAxPress(input, session);
    case "open_app": return handleOpenApp(input, session);
    case "request_access": return handleRequestAccess(input, session);
    case "list_apps": return handleListApps(session);
    case "clipboard_read": return handleClipboardRead(session);
    case "clipboard_write": return handleClipboardWrite(input, session);
    case "batch": return handleBatch(input, session);
    default: return fail(`Unknown action: ${(input as any).action}`);
  }
}
