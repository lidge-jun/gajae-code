...
function text(t: string): TextContent { return { type: "text", text: t }; }
...
function findDisplayByName(displays: DisplayInfo[], query: string): DisplayInfo | undefined {
  ...
  return displays.find(d => d.name?.toLowerCase() === q)
    ?? displays.find(d => d.name?.toLowerCase().includes(q));
}
...
async function displayIdFor(session: SessionState): Promise<number | undefined> {
  ...
  try {
    ...
    for (const d of displays) {
      if (
        mouse.x >= d.originX && mouse.x < d.originX + d.width &&
        mouse.y >= d.originY && mouse.y < d.originY + d.height
      ) {
        return d.id;
      }
    }
...
export function registerScreenshotTools(
    ...
    "screenshot",
    {
      ...
      inputSchema: z.object({
        save_to_disk: z.boolean().optional().describe(
          "Save the image to disk and return the path. Only set when sharing the image with the user.",
        ),
      }),
    },
    async ({ save_to_disk }) => {
  ...
  server.registerTool(
    ...
    {
      description:
        "Take a higher-resolution screenshot of a region from the most recent screenshot.",
      inputSchema: z.object({
        region: z
          .tuple([z.number(), z.number(), z.number(), z.number()])
          .describe(
            "(x0, y0, x1, y1) in the coordinate space of the most recent full screenshot.",
          ),
        save_to_disk: z.boolean().optional().describe(
          "Save the image to disk and return the path. Only set when sharing the image with the user.",
        ),
  ...
  server.registerTool(
    ...
    async ({ display }) => {
      ...
      if (!match) {
        ...
        return fail(`Monitor "${display}" not found. Available monitors: [${names}]. Pass the exact name from the screenshot note.`);
      }
