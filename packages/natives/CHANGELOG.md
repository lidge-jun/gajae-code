...
- Updated native module loader to check `XDG_DATA_HOME` environment variable for native addon location before falling back to `~/.jwc/natives`
...
- Added automatic extraction of embedded native addon to `~/.jwc/natives/<version>` on first run for compiled binaries
...
- Embedded native addon payload for compiled binaries and extract to `~/.jwc/natives/<version>` on first run
