/**
 * jwc fork: composer models (xai grok-composer-*, cursor composer-*) get the
...
describe("openai-completions composer discipline injection", () => {
	...
	it("does not inject for non-composer models", () => {
		...
		expect(serialized).not.toContain("File-editing discipline");
	});
...
describe("cursor composer discipline injection", () => {
	it("adds the discipline block after the host override for composer model ids", () => {
		...
		expect(contents[1]).toBe(COMPOSER_EDIT_DISCIPLINE_PROMPT);
		expect(contents[2]).toBe("Host system prompt.");
	});
	...
	it("does not inject discipline without a host system prompt", () => {
		...
		const contents = jsons.map(json => (JSON.parse(json) as { content: string }).content);
		expect(contents).toHaveLength(2);
		expect(contents[1]).toBe("You are a helpful assistant.");
	});
});
