...
describe("GJC native skill-state hooks", () => {
	...
	it("rich jaw-interview prompt activation blocks product mutation and direct spec artifacts", async () => {
		...
		expect(blockedSpec.reason).toBe("jwc-target");
		expect(blockedSpec.message).toContain("runtime-owned");

		const blockedJwcBash = await getJawInterviewMutationDecision({
			cwd: root,
			sessionId: "session-rich",
			tool: { name: "bash" } as never,
			args: { command: "cat sample.md > .jwc/specs/jaw-interview-sample.md" },
		});
		expect(blockedJwcBash.blocked).toBe(true);
		expect(blockedJwcBash.reason).toBe("jwc-target");
...
`,
	...
	it("Stop blocks while matching skill state is active and allows terminal mode state", async () => {
		...
		expect(blocked.outputJson).toMatchObject({ decision: "block", stopReason: "jwc_skill_ralplan_planner" });
