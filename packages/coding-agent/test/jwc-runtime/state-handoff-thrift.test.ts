...
import { createUltragoalPlan, runNativeUltragoalCommand } from "@gajae-code/coding-agent/jwc-runtime/ultragoal-runtime";
...
describe("CONSUMER/KEY-FIELD MATRIX for compact handoff payloads", () => {
	...
	it("documents the ralplan receipt-only guideline for role agents", async () => {
		const skillDoc = await fs.readFile(
			path.join(repoRoot, "packages/coding-agent/src/defaults/jwc/skills/ralplan/SKILL.md"),
			"utf-8",
		);
