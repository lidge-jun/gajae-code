...
import { normalizeLegacyState } from "../../src/jwc-runtime/state-migrations";
...
describe("G7 gjc state migration gate", () => {
	...
	it("migrates tampered state with --force and audits the forced mismatch", async () => {
		await withTempCwd(async cwd => {
			...
			expect(persisted.receipt).toMatchObject({ owner: "jwc-state-cli", status: "fresh" });
			...
			expect(mismatch).toMatchObject({ skill: "ralplan", category: "state", owner: "jwc-state-cli" });
