...
describe("state-schema (A1)", () => {
	...
	it("RequiredOnWriteEnvelopeSchema requires anchored fields incl. stamped receipt", () => {
		const validReceipt = {
			...
			skill: "ralplan",
			owner: "jwc-state-cli",
			command: "jwc state ralplan write",
			state_path: "/x/.jwc/state/ralplan-state.json",
			storage_path: "/x/.jwc/state/ralplan-state.json",
...
describe("readJwcJson (A2)", () => {
	it("returns null for a missing file", async () => {
		const result = await readJwcJson(path.join(os.tmpdir(), "gjc-nope-xyz.json"), WorkflowStateEnvelopeSchema);
	...
	it("returns ok for valid content and preserves raw", async () => {
		...
		const result = await readJwcJson(file, WorkflowStateEnvelopeSchema);
