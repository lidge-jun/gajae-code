...
const skillFiles = CANONICAL_JWC_WORKFLOW_SKILLS.map(skill =>
	path.join(repoRoot, "packages", "coding-agent", "src", "defaults", "jwc", "skills", skill, "SKILL.md"),
);
...
describe("workflow command-reference proof spike", () => {
	it("renders deterministic command-reference blocks", () => {
		...
		expect(first.bytes).toContain("jwc state ralplan write --input");
	...
	it("reports byte-equal or gap status without mutating SKILL.md files", async () => {
		...
		expect(parsed.results).toHaveLength(CANONICAL_JWC_WORKFLOW_SKILLS.length);
		for (const item of parsed.results) {
			expect([...CANONICAL_JWC_WORKFLOW_SKILLS] as string[]).toContain(item.skill);
	...
	it("prints per-skill token budget report", async () => {
		...
		expect(parsed.skills).toHaveLength(CANONICAL_JWC_WORKFLOW_SKILLS.length);
