...
import { type LoadedSubskillActivation, loadJwcPlugin, toActiveSubskillEntry } from "../src/extensibility/jwc-plugins";
...
async function activationFromFixture(cwd: string): Promise<LoadedSubskillActivation> {
	const plugin = await loadJwcPlugin(path.join(cwd, ".jwc", "gjc-plugins", "valid-skill-plugin"));
