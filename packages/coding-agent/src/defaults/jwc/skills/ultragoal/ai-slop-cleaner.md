...
- Recursion guard: you are already inside an Ultragoal workflow. Do NOT spawn nested `orchestrate`, `team`, `jaw-interview`, or `ultragoal` workflows. Broad, ambiguous, cross-layer, or architectural findings are handed to the leader as review blockers, not resolved here.
...
Recursion Guard: [confirmed no nested orchestrate/team/jaw-interview/ultragoal spawned; broad findings handed to leader]
