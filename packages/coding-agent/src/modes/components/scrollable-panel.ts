...
/**
 * Docked read-once panel (99.20.07 P2 — generalization of the /quota panel
 * shape). Mounted in place of the editor via SelectorController.showSelector
 * so one-shot reports (/changelog, /hotkeys, /tools, /context, /session,
 * /jobs, /memory view) render as a scrollable dock instead of being appended
 * to the chat transcript. Content arrives asynchronously: the panel shows a
 * loading line until `setContent`/`setMessage` is called.
 *
 * NOTE: the parallel-landed QuotaPanelComponent shares this shape; folding it
 * onto this base is tracked as parallel-landing cleanup (99.00.04 §4).
 */
export class ScrollablePanelComponent extends Container {
	#title: string;
	#close: () => void;
	#requestRender: () => void;
	...
	/** Swap the loading state for content rendered at the current width. */
	...
	override render(width: number): string[] {
		...
		out.push(
			theme.fg("dim", maxScroll > 0 ? "  up/down scroll  pgup/pgdn page  g/G top/bottom  esc close" : "  esc close"),
		);
		return out;
	}
}
