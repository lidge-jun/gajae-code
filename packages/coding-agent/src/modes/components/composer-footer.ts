import { type Component, truncateToWidth } from "@gajae-code/tui";
...
export class ComposerFooter implements Component {
	...
	render(width: number): string[] {
		...
		if (this.#transient) {
			...
			line = theme.fg("accent", sanitizeStatusText(this.#mode));
		} else if (this.#hint) {
			line = theme.fg("dim", sanitizeStatusText(this.#hint));
		}
		// Reserve the row even when empty — constant composer geometry (083.7).
