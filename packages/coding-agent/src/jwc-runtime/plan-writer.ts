 ...
 *
 * 1. **Consensus handoff**: `jwc ralplan [--interactive] [--deliberate] [--architect <kind>]
 ...
 *
 * 2. **Artifact write**: `jwc ralplan --write --stage <type> --stage_n <N> --artifact
...
async function persistActiveRunId(cwd: string, sessionId: string | undefined, runId: string): Promise<void> {
	...
	await writeWorkflowEnvelopeAtomic(statePath, existing, {
		cwd,
		receipt: { cwd, skill: "ralplan", owner: "jwc-runtime", command: "jwc ralplan persist-run-id", sessionId },
		audit: { category: "state", verb: "write", owner: "jwc-runtime", skill: "ralplan" },
	});
}
...
async function applyPlannerStateUpdate(
	...
	await writeWorkflowEnvelopeAtomic(statePath, existing, {
		cwd,
		receipt: { cwd, skill: "ralplan", owner: "jwc-runtime", command: "jwc ralplan planner-state", sessionId },
		audit: { category: "state", verb: "write", owner: "jwc-runtime", skill: "ralplan" },
	});
}
...
async function persistArtifact(resolved: ResolvedArtifactArgs, cwd: string): Promise<PersistedArtifact> {
	...
	await writeArtifact(filePath, content, {
		cwd,
		audit: { category: "artifact", verb: "write", owner: "jwc-runtime", skill: "ralplan" },
	});
	...
	await appendJsonl(path.join(runDir, "index.jsonl"), indexEntry, {
		cwd,
		audit: { category: "ledger", verb: "append", owner: "jwc-runtime", skill: "ralplan" },
	});
...
async function seedRalplanState(
	...
	// Reuse an existing run id when present so a re-invocation of `jwc ralplan "task"` doesn't
