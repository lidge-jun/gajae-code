...
const strictSkillEnum = z.enum([...CANONICAL_JWC_WORKFLOW_SKILLS]);
 ...
 */
export type ReadJwcJsonResult<T> = { ok: true; value: T; raw: unknown } | { ok: false; error: string; raw: unknown };
...
/**
 * Parse + schema-validate a `.jwc` JSON file at the read boundary.
 ...
 */
export async function readJwcJson<T>(filePath: string, schema: z.ZodType<T>): Promise<ReadJwcJsonResult<T> | null> {
	let text: string;
	try {
