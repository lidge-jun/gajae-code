...
function canonicalSkillOrThrow(skill: string): CanonicalJwcWorkflowSkill {
...
function legacyPhaseForSkill(skill: CanonicalJwcWorkflowSkill, phase: string): string {
	if (phase === "planning") return initialPhaseForSkill(skill);
	return phase;
}

function normalizePhase(skill: CanonicalJwcWorkflowSkill, value: unknown): string {
...
function receiptWithRequiredFields(raw: unknown, skill: CanonicalJwcWorkflowSkill): Record<string, unknown> {
	const receipt =
	...
	receipt.skill = skill;
	// 260613 flip: legacy gjc-era owners normalize to jwc-*; unknown values reset to the CLI owner.
	if (typeof receipt.owner === "string") receipt.owner = receipt.owner.replace(/^gjc-/, "jwc-");
	if (receipt.owner !== "jwc-state-cli" && receipt.owner !== "jwc-runtime" && receipt.owner !== "jwc-hook") {
...
function migrateV1ToV2(state: Record<string, unknown>, skill: CanonicalJwcWorkflowSkill): Record<string, unknown> {
...
export async function migrateAndPersistLegacyState(
	...
	const persistedPath = await writeWorkflowEnvelopeAtomic(args.statePath, state, {
		...
		receipt: {
			...
			owner: "jwc-state-cli",
			command: `jwc state ${canonicalSkill} migrate`,
