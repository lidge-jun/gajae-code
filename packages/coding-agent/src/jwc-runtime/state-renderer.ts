...
export function renderContractMarkdown(skill: CanonicalJwcWorkflowSkill, contract: unknown): string {
...
export function renderGoalStatusMarkdown(summary: {
	...
	if (!summary.exists)
		return `# ultragoal status\n\n- status: missing\n- No ultragoal plan found at ${summary.paths.goalsPath}. Run \`jwc ultragoal create-goals --brief "..."\` first.\n`;
	...
	if (summary.jwcObjective) lines.push(`- objective: ${summary.jwcObjective}`);
