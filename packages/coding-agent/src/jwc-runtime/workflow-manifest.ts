...
const COMMON_TYPED_ARGS: TypedArgSpec[] = [
	...
	{ name: "mode", type: "enum", enumValues: [...CANONICAL_JWC_WORKFLOW_SKILLS], appliesToVerbs: [...STATE_VERBS] },
	...
	{ name: "turn-id", type: "string", appliesToVerbs: ["write", "clear", "handoff"] },
	{
		name: "to",
		type: "enum",
		enumValues: [...CANONICAL_JWC_WORKFLOW_SKILLS],
		required: true,
	...
	{ name: "skill", type: "enum", enumValues: [...CANONICAL_JWC_WORKFLOW_SKILLS], appliesToVerbs: ["doctor"] },
...
export const WORKFLOW_MANIFEST: Record<CanonicalJwcWorkflowSkill, SkillManifest> = {
...
export function getSkillManifest(skill: CanonicalJwcWorkflowSkill): SkillManifest {
	return WORKFLOW_MANIFEST[skill];
}

export function isKnownWorkflowState(skill: CanonicalJwcWorkflowSkill, state: string): boolean {
	return WORKFLOW_MANIFEST[skill].states.some(entry => entry.id === state);
}

export function isValidTransition(skill: CanonicalJwcWorkflowSkill, from: string, to: string): boolean {
	if (from === to) return true;
	return WORKFLOW_MANIFEST[skill].transitions.some(transition => transition.from === from && transition.to === to);
}

export function listVerbs(skill: CanonicalJwcWorkflowSkill): string[] {
	return WORKFLOW_MANIFEST[skill].verbs.map(verb => verb.name);
}

export function typedArgsFor(skill: CanonicalJwcWorkflowSkill, verb: string): TypedArgSpec[] {
