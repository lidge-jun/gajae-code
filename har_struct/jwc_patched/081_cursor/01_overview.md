# 081_cursor — cursor 프로바이더·도구군 (jwc_patched)

> **MOC 정본**: `devlog/_plan/260612_jawcode_fork/081_moc_cursor_tools.md`
...
# 081_cursor — Cursor 프로바이더 / 도구 렌더 (jwc_patched)

**🟡 부분 수정** — fork에서 cursor provider 패치 진행 (081.1–081.7).

- ToolCall 파싱·렌더 경로 수정
- host override fix (081.6)
- kiro/ACP 통합은 091/092 별도 밴드
...
- `081.1_issue_toolcall_render.md` — # 081.1 — issue: TUI에 도구 호출이 하나도 안 그려지는 문제 (cursor 파서 드롭)
- `081.2_issue_title_hallucination.md` — # 081.2 — issue: 환각 세션 타이틀 저장 (title-generator 환각)
- `081.3_issue_exec_unbound.md` — # 081.3 — issue: cursor 도구 실행이 전부 실패 (exec 핸들러 unbound this)
- `081.4_issue_glob_empty_pattern.md` — # 081.4 — issue: Glob 호출 시 "Pattern must not be empty"
- `081.5_audit_unbound_elsewhere.md` — # 081.5 — audit: cursor 외 동형 패턴(unbound-this / oneof-drop / arg-mismatch) 전역 감사
- `081.6_fix_cursor_host_override.md` — # 081.6 — fix: cursor 주입구에 host-override 지시 (cursor 설정 무시 → jwc 설정 우선)
- `081.7_issue_cursor_autocompact.md` — # 081.7 — issue: cursor에서 자동 compact가 안 터짐 (usage.input=0)

## 3. gjc_origin ↔ jwc_patched delta

| 항목 | gjc_origin | jwc_patched |
|---|---|---|
| 1 | cursor provider baseline | 081.1-4 tool render ✅ |
| 2 | ToolCall oneof issues known | 081.7 compact ✅ |
| 3 | — | PR #515 pending |
